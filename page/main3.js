...
var favRouter = require('./routes/topic1');
...
app.use(express.static('public/topic'));
app.use(express.static('public/topic/update'));
...
app.use((request, response, next) => {
  fs.readdir('./data1', function (err, filelist1) {
    request.list2 = filelist1;
    next();
  });
});
